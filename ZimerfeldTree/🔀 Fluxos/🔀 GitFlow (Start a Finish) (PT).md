...
- **Feature filha de feature** via *based on:* → *finish* **cascateia** até `develop`. Ver [[🌿 GitFlow flexível — feature filha de feature (PT)|🌿 GitFlow flexível — feature filha de feature]].
- **Erro de merge (conflito):** o plugin para e mostra o resultado; resolver com `git merge --abort` ou resolver + `git commit`.

> Mapa completo Start→git puro (todos os tipos) em [[👁️ Visão Geral (PT)|👁️ Visão Geral]].

## Relacionado

- [[🌲 Abrir e navegar a árvore (PT)|🌲 Abrir e navegar a árvore]]
