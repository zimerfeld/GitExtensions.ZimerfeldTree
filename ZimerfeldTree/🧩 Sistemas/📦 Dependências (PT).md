...
> [!abstract] Resumo
> Programas necessários para **usar** e para **compilar** o plugin. O detalhamento passo-a-passo (downloads, verificação, procedimento de instalação) vive na nota de ferramentas [[📦 Dependências do ZimerfeldTree (PT)|📦 Dependências do ZimerfeldTree]] — esta nota é o resumo de sistema equivalente à do vault irmão.
...
> [!info]
> Todos os comandos GitFlow (Start, Publish, Track, Update, Finish) usam **git puro**. O binário `git-flow` **não** precisa estar instalado. Ver [[🔀 GitFlow em git puro (PT)|🔀 GitFlow em git puro]].

## Relacionado

- [[📦 Dependências do ZimerfeldTree (PT)|📦 Dependências do ZimerfeldTree]] — versão detalhada, passo-a-passo
- [[🏗️ Arquitetura (PT)|🏗️ Arquitetura]]
- [[👁️ Visão Geral (PT)|👁️ Visão Geral]]
- [[🔧 build.ps1 (PT)|🔧 build.ps1]]
