...
## Decisão
Implementar **todos** os comandos GitFlow (start / publish / track / update / finish, para feature/bugfix/release/hotfix/support) como **sequências de git nativo** dentro do [[⚙️ BranchHierarchyService|BranchHierarchyService]]. O binário `git-flow` **não** precisa estar instalado.
...
- Chaves `gitflow.*` de config existem só para compatibilidade visual/CLI — ver [[⚙️ git flow - chaves de config (CLI)]].
