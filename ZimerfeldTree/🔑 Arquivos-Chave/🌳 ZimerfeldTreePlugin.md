...
Classe exportada via MEF (`[Export(typeof(IGitPlugin))]`), herdando de `GitPluginBase`. É o objeto que o **GitExtensions** instancia e lista no menu **Plugins → ZimerfeldTree**. Ver [[🧩 Plugin MEF para GitExtensions]].
...
| Ícone | Usa `PluginIcon` (`Resources/ico.png`, "Árvore da Vida") — ver [[🖼️ PluginIcon]]. |

## Notas

- O ícone e o título das janelas mantêm sempre o prefixo **`ZimerfeldTree - `** (`BranchHierarchy` / `GitFlow` / `Restore`).
- O working directory **não** vem do repositório ativo do host — vem do `cboRepo` da janela. Ver [[🪟 Janela não-modal singleton]].
