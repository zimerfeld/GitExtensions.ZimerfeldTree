...
Toda a lógica git da janela principal e do GitFlow mora aqui. Roda `git` num `ProcessStartInfo` (stdout/stderr redirecionados, UTF-8, sem janela), parseia a saída e devolve modelos [[🌿 BranchNode (PT)|BranchInfo]].
...
Implementa as sequências de start / publish / track / update / finish para os cinco tipos. O mapa completo **Start→git** está em [[👁️ Visão Geral (PT)|👁️ Visão Geral]] e [[🔀 GitFlow em git puro (PT)|🔀 GitFlow em git puro]].

## Limitações conhecidas

- **Duas branches no mesmo commit** não formam pai-filho (o BFS nunca encontra uma como pai da outra) → ambas viram raízes; solução automática = commit vazio no Start com *based on*. Ver [[🌿 Hierarquia de branches — branches no mesmo commit (PT)|🌿 Hierarquia de branches — branches no mesmo commit]].
- **Branch real não pode ser nó-pai** de outra (o ref seria arquivo **e** diretório).

## Relacionado

- [[🪟 BranchHierarchyForm (PT)|🪟 BranchHierarchyForm]]
- [[🔀 GitFlowForm (PT)|🔀 GitFlowForm]] · [[⏪ RestoreForm (PT)|⏪ RestoreForm]]
