...
> O conteúdo do README (funcionalidades, dependências, instalação, estrutura e limitações) vive aqui no cofre. Os **fluxos detalhados de cada janela** estão em [[🌳 Interface ZimerfeldTree — botões e fluxos (PT)|🌳 Interface ZimerfeldTree — botões e fluxos]], [[🔀 Interface GitFlow — botões e fluxos (PT)|🔀 Interface GitFlow — botões e fluxos]] e [[⏪ Interface Restore — botões e fluxos (PT)|⏪ Interface Restore — botões e fluxos]].
...
- **Plugin model:** MEF (`System.ComponentModel.Composition`) — ver [[🧩 Plugin MEF para GitExtensions (PT)|🧩 Plugin MEF para GitExtensions]]
...
- **Push protegido contra divergência** — se a branch a enviar está **atrás** do remoto, um aviso oferece **Baixar com rebase e então enviar automaticamente** (`git pull --rebase` reaplica os commits locais por cima dos remotos, sem commit de merge → branch fast-forward → push aceito); rebase com conflito é reportado e o push é pulado, evitando a rejeição `non-fast-forward`
...
- **Botão GitFlow Initialize** — aplica de uma vez as chaves `gitflow.*` padrão (ver [[⚙️ git flow - chaves de config (CLI) (PT)|⚙️ git flow - chaves de config (CLI)]])
...
> Detalhes controle-a-controle: [[🌳 Interface ZimerfeldTree — botões e fluxos (PT)|🌳 Interface ZimerfeldTree — botões e fluxos]] · [[🔀 Interface GitFlow — botões e fluxos (PT)|🔀 Interface GitFlow — botões e fluxos]] · [[⏪ Interface Restore — botões e fluxos (PT)|⏪ Interface Restore — botões e fluxos]].
...
Ver também [[📦 Dependências do ZimerfeldTree (PT)|📦 Dependências do ZimerfeldTree]].
...
- **Duas branches no exato mesmo commit não formam pai-filho** — o BFS de ancestralidade nunca encontra uma como pai da outra; ambas viram raízes. Solução automática: commit vazio no Start com **based on**. Detalhe em [[🌿 Hierarquia de branches — branches no mesmo commit (PT)|🌿 Hierarquia de branches — branches no mesmo commit]].
...
> O GitExtensions grava config no formato interno dele, mas o git flow CLI espera outras chaves. Solução em [[⚙️ git flow - chaves de config (CLI) (PT)|⚙️ git flow - chaves de config (CLI)]].

## 🔢 Versionamento
- Versão atual: **1.0.361** (README + csproj + nuspec + vault em sincronia)
...
- **[GitExtensions.ZimerfeldCommitMsg](https://www.nuget.org/packages/GitExtensions.ZimerfeldCommitMsg)** — gera automaticamente a mensagem de commit (Conventional Commits) resumindo os arquivos staged. GitHub: [zimerfeld/GitExtensions.ZimerfeldCommitMsg](https://github.com/zimerfeld/GitExtensions.ZimerfeldCommitMsg).
- **[GitExtensions.ZimerfeldLFS](https://github.com/zimerfeld/GitExtensions.ZimerfeldLFS)** — gerencia o Git LFS (Large File Storage): rastrear, enviar e baixar arquivos binários grandes diretamente pela interface do GitExtensions.

## 🔗 Relacionado
- [[🌳 Interface ZimerfeldTree — botões e fluxos (PT)|🌳 Interface ZimerfeldTree — botões e fluxos]]
- [[🔀 Interface GitFlow — botões e fluxos (PT)|🔀 Interface GitFlow — botões e fluxos]]
...
- [[⚙️ git flow - chaves de config (CLI) (PT)|⚙️ git flow - chaves de config (CLI)]]
- [[📦 Dependências do ZimerfeldTree (PT)|📦 Dependências do ZimerfeldTree]]
