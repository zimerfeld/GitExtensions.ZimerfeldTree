...
![[Anexos/ScreenShots/ScreenshotGitFlow.png]]
...
- **Feature filha de feature** via *based on:* → *finish* **cascateia** até `develop`. Ver [[../Decisoes/GitFlow flexível — feature filha de feature]].
- **Erro de merge (conflito):** o plugin para e mostra o resultado; resolver com `git merge --abort` ou resolver + `git commit`.

> Mapa completo Start→git puro (todos os tipos) em [[../Sistema/Visão Geral]].
