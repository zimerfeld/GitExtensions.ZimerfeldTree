...
- **Idioma por janela**: o `cboLanguage` agora é **por janela** (campo `_lang`), persistido individualmente (chave `language` no `ZimerfeldRestore.settings.json`) e recarregado no `Load` via `ApplyLanguage()` usando `I18n.Load(scope, _lang)`. **Não** chama mais `I18n.SetLanguage` (que é global) — só a BranchHierarchy escreve no `ZimerfeldTree.language.json`. Na primeira abertura (sem valor salvo) herda `I18n.Current` (idioma da janela principal).
