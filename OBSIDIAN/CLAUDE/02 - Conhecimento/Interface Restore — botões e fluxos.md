...
> [!abstract] Resumo
> Janela **modal** (`RestoreForm`) que dirige operações de restauração de histórico git: restaurar arquivo de commit, cherry-pick e reset de branch. Acessível via botão **Voltar Versão** da [[Interface ZimerfeldTree — botões e fluxos]]. Projeto: [[GitExtensions.ZimerfeldTree]].
...
3. Exibe resultado. Sucesso → `RepoMutated` (ZimerfeldTree atualiza a árvore em background).

### Botão Cherry-Pick (`_btnCherryPick`) → `DoCherryPick`
1. Lê campo `Commit(s)`:
   - Hash simples → `git cherry-pick <hash>`.
   - Range com `..` → `git cherry-pick <antigo>..<recente>`.
2. Exibe resultado. Sucesso → `RepoMutated`.
...
6. Exibe resultado. Sucesso → `RepoMutated`.
...
- Os últimos valores de cada campo são **persistidos** automaticamente ao fechar em `ZimerfeldRestore.settings.json` e restaurados na próxima abertura.
